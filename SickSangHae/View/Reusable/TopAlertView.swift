  ...
  @ObservedObject var viewModel: TopAlertViewModel
    ...
    var body: some View {
      ...
      case .shortTermUnEaten:
        TopAlertBaseView(iconImage: "img_eat", message: "\(viewModel.name) 항목이 일반으로 이동됐어요", backgroundColor: .pointRLight, strokeColor: .pointRMiddle, viewModel: viewModel)
      case .shortTermPinned:
        TopAlertBaseView(iconImage: "img_eat", message: "\(viewModel.name) 항목이 빨리 먹어야해요로 이동됐어요", backgroundColor: .pointRLight, strokeColor: .pointRMiddle, viewModel: viewModel)
      case .longTermUnEaten:
        TopAlertBaseView(iconImage: "img_eat", message: "\(viewModel.name) 항목이 장기 보관으로 이동됐어요", backgroundColor: .alertGreen, strokeColor: .primaryGMiddle, viewModel: viewModel)
      default:
